    ...
    def __data_generation(self, list_IDs_temp):
        ...
        for index, id in enumerate(list_IDs_temp):
            ...
            if ((temp.shape[0] == 2656) and (temp.shape[1] ==1494)):
                temp = cv2.resize(temp, (1504, 2688)) #hecho con mn
            elif ((temp.shape[0] == 1494) and (temp.shape[1] == 2656)): 
                temp = cv2.resize(temp, (2688, 1504)) #hecho con mn
            if ((temp.shape[0] == 1728) and (temp.shape[1] ==2304)): 
                temp = cv2.resize(temp, (2336, 1760)) #hecho con mn
            if ((temp.shape[0] == 1824) and (temp.shape[1] ==2736)):
                temp = cv2.resize(temp, (2752, 1856)) #hecho con mn
            if ((temp.shape[0] == 1728) and (temp.shape[1] ==1728)):
                 temp = cv2.resize(temp, (1760, 1760)) #hecho con mn
